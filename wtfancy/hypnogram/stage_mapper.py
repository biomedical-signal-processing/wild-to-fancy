...
from wtfancy import Defaults_hyp
...
def check_number_match(ss):
    ...
    if len(numbers) not in (0, 1):
        ...
        num = numbers[0]
        valid_map = {1: Defaults_hyp.NON_REM_STAGE_1[0],
                     2: Defaults_hyp.NON_REM_STAGE_2[0],
                     3: Defaults_hyp.NON_REM_STAGE_3[0],
                     4: Defaults_hyp.NON_REM_STAGE_3[0]}
        assert np.all(np.in1d(list(valid_map.values()),
                              list(Defaults_hyp.get_stage_string_to_class_int().keys())))
        if num in valid_map:
            possible_match = True
...
def check_wake_match(ss):
    ...
    if any(in_string):
        possible_match, match_value = True, Defaults_hyp.AWAKE[0]
    return possible_match, match_value
...
def check_REM_match(ss):
    ...
    in_string = [s in ss for s in valid_substrings]
    if any(in_string):
        possible_match, match_value = True, Defaults_hyp.REM[0]
    return possible_match, match_value
...
def check_unknown_match(ss):
    ...
    in_string = [s in ss for s in valid_substrings]
    if any(in_string):
        possible_match, match_value = True, Defaults_hyp.UNKNOWN[0]
    return possible_match, match_value
    ...
    if n_matches == 1:
        ...
        print("[OBS]: Mapping variable stage string '{:s}' to stage '{}' "
              "(class int {})".format(stage_string, match,
                                      Defaults_hyp.get_stage_string_to_class_int()[match]))
        return match
    elif n_matches == 0:
...
def stage_string_to_class(stage_string):
    return Defaults_hyp.get_stage_string_to_class_int()[stage_string.upper()]
