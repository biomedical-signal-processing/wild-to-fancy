...
import numpy as np
from wtfancy.hypnogram import SparseHypnogram
from wtfancy.hypnogram.formats import StartDurationStageFormat
from wtfancy.hypnogram.stage_mapper import create_variable_ann_to_class_int_dict
...
def fill_hyp_gaps(init_times_sec, durations_sec, stages, fill_value):
    ...
    assert len(init_times_sec) == len(durations_sec) == len(
        stages), "Inits, durations and stages must have equal length"
    actual_diffs = np.diff(init_times_sec)
    gap_lengths = actual_diffs - durations_sec[:-1]  # gap lenghts (all - null, positive and negative)
    gap_inds = np.where(gap_lengths)[0]  # gap indices (positive and negative)

    duplicate_stages_equal = 0
    duplicate_stages_unknown = 0
    series_occ = 0
    # Negative gaps
    if sum(gap_lengths < 0) != 0:  # negative gaps found
        # Check negative gaps type (duplicate stages or 2 series)
        for i in np.where(gap_lengths < 0)[0]:
            if init_times_sec[i] == init_times_sec[i+1]:
                if durations_sec[i] == durations_sec[i+1]:                                                              # Case 1: two sleep stages for the same epoch
                    if stages[i] != stages[i+1]:  # if sleep stages are different, set to UNKNOWN
                        stages[i] = fill_value
                        duplicate_stages_unknown += 1
                    else:
                        duplicate_stages_equal += 1
                else:
                    raise NotImplementedError("The implementation has not yet been tested for its handling "
                                              "of this situation.")
            else:                                                                                                       # Case 3: two series of annotations
                series_occ += 1
    ...
    if sum(gap_lengths > 0) != 0:
        print(f'-- Gaps filled: {sum(gap_lengths[gap_lengths > 0]) / 60:.3}min '
              f'({sum(gap_lengths[gap_lengths > 0]) / 3600:.3}h)')
    # Correct all gaps
    init_times_sec, durations_sec, stages = map(list, (init_times_sec, durations_sec, stages))
    slide_counter = 0
    for ind in gap_inds:
        if ind >= len(gap_lengths):  # if ind == 0 or ind >= len(gap_lengths):
            raise NotImplementedError("The implementation has not yet been tested for its handling "
                                      "of this situation. Please raise an issue on GitHub.")
        if gap_lengths[ind] > 0:  # positive gap
            # Insert gap section
            length = gap_lengths[ind]
            init_times_sec.insert(ind+1+slide_counter,
                                  init_times_sec[ind+slide_counter] + durations_sec[ind+slide_counter])
            durations_sec.insert(ind+1+slide_counter, length)
            stages.insert(ind+1+slide_counter, fill_value)
            slide_counter += 1
        else:  # negative gap
            if duplicate_stages_equal+duplicate_stages_unknown:
                init_times_sec.pop(ind + 1 + slide_counter)
                durations_sec.pop(ind + 1 + slide_counter)
                stages.pop(ind + 1 + slide_counter)
                slide_counter -= 1
            else:
                raise NotImplementedError("The implementation has not yet been tested for its handling "
                                          "of this situation.")
    assert not hyp_has_gaps(init_times_sec, durations_sec)
...
def load_events_file(events_file_path):
    ...
    """
    from wtfancy.io.annotations.ann_extractors import extract_from_start_dur_stage
